...
import static com.faojen.exoterra.blocks.superiorpowerbank.SuperiorPowerBankBE.SUP_BANK_CAPACITY_PUB;
...
public class SuperiorPowerBankScreen extends AbstractContainerScreen<SuperiorPowerBankContainer> {
	private static final ResourceLocation background = new ResourceLocation(ExoTerra.MODID,
			"textures/gui/superior_power_bank.png");

	private final SuperiorPowerBankContainer container;

	public SuperiorPowerBankScreen(SuperiorPowerBankContainer container, Inventory playerInventory, Component title) {
	...
	public void render(PoseStack stack, int mouseX, int mouseY, float partialTicks) {
		...
		if (mouseX > (leftPos + 9) && mouseX < (leftPos + 9) + 158 && mouseY > (topPos + 26)
			...
			this.renderTooltip(stack, new TranslatableComponent(
					...
					MagicHelpers.withSuffix(SUP_BANK_CAPACITY_PUB)), mouseX, mouseY);	
		}
