package com.faojen.exoterra.blocks.simple.compowerbank;
...
public class CommonPowerBankContainer extends AbstractContainerMenu {
    ...
    public CommonPowerBankContainer(int windowId, Inventory playerInventory, FriendlyByteBuf extraData) {
        this((CommonPowerBankBE) playerInventory.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(2), windowId, playerInventory, new ItemStackHandler(0));
    }

    public CommonPowerBankContainer(@Nullable CommonPowerBankBE tile, ContainerData commonPowerBankData, int windowId, Inventory playerInventory, ItemStackHandler handler) {
        super(Registration.COMMON_POWER_BANK_CONTAINER.get(), windowId);

        this.handler = handler;
        this.tile = tile;

        this.data = commonPowerBankData;
        this.setup(playerInventory);

        addDataSlots(commonPowerBankData);
    }
